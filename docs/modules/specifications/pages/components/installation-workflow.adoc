...
* **At the org (owner) level:** Offer **org-level Infrastructure as Code** setup: create or link an `infra` (or `infrastructure`) repo for the org. Offer **Pulumi** (TypeScript) from `dev-centr/infra-template` **or** OpenTofu; prefer Pulumi when following Business Bootstrap. Also offer **Org email / messaging bootstrap** linking to the Business Bootstrap email matrix (human vs product lanes stay separate). This is the only place where org-level IaC setup is offered.
...
* **Org + owner:** IaC flow is **org-level** infra repo (create/link). Prefer **Pulumi** from `infra-template` when the org follows Business Bootstrap; still offer **OpenTofu** as a compatible alternative. Surface a separate **Org email / messaging bootstrap** entry that opens Business Bootstrap docs (human ZeptoMail vs product Resend/Customer.io — never one wizard that picks a single ESP for everything).
...
.. **Org-level infra repo:** One central repo (e.g. `infra` or `infrastructure`) for the org; app repos typically have no IaC or only thin wrappers. Best for consistency and governance. Preferred bootstrap: **Pulumi** TypeScript from `dev-centr/infra-template` (includes Antora `docs/`). OpenTofu remains a valid alternative.
.. **Per-project IaC:** This repo gets its own IaC (e.g. `infra/` with OpenTofu or Pulumi). Best for team autonomy or isolated services.
...
.. Recommend **OpenTofu** by default for per-project HCL, or **Pulumi** if the org already standardized on it.
.. Offer to bootstrap a minimal layout and optionally install the chosen CLI if missing.
.. Link to the Docs page for full patterns (registration, thin modules, remote state, policy as code).

== Org email / messaging bootstrap (org owner)

At the **organization** owner node, offer a guided add (wizard shorthand) for **email and messaging services**. This is documentation- and checklist-driven in the first version (no mandatory cloud API writes):

* Present the four ITIL-shaped services separately: Email Inbound, Email Outbound Human, Email Transactional App, Marketing Journeys (see xref:business-bootstrap::explanation/itil-email-services.adoc[ITIL-shaped email services]).
* Deep-link Docs to xref:business-bootstrap::explanation/email-decision-matrix.adoc[Email decision matrix] and xref:business-bootstrap::explanation/human-vs-product-messaging.adoc[Human vs product messaging].
* Never recommend Resend for human Gmail send-as; never recommend Brevo as the default for product journey orgs.
* When DNS changes are needed, point operators at the org `infra` Pulumi stack (create it first if missing).

Full UI automation of ESP portals is out of scope for this pass.
...
* xref:components/iac-discovery.adoc[IaC Discovery] — how IaC is discovered and mapped in repos.
* xref:components/org-setup-wizard.adoc[Org setup wizard] — org-owner bootstrap for infra + email lanes.
* xref:general-knowledge::explanation/infrastructure/iac.adoc[Infrastructure as Code (IaC)] — personal vs organization use (Docs button target).
* xref:business-bootstrap::explanation/org-infra-repo-pattern.adoc[Org infra repo pattern]
* xref:business-bootstrap::explanation/email-decision-matrix.adoc[Email decision matrix]
