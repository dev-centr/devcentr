...
`Arrangement` opens the **AI stack topology canvas** (Role Topology Composer, `ai-stack` pack): blank map landing, layout recipes, and **Add an AI tool…** guided add. Full behavior: xref:components/ai-workflow-arrangement.adoc[AI Workflow Arrangement] and xref:components/role-topology-composer.adoc[Role Topology Composer].
...
* xref:components/ai-workflow-arrangement.adoc[AI Workflow Arrangement] / xref:components/role-topology-composer.adoc[Role Topology Composer] — Arrangement tab opens the AI stack canvas (not a one-shot wizard; guided add is separate)
* xref:components/agent-shell-registry.adoc[Agent Shell Registry] supplies arrangement ids and shopping data.
