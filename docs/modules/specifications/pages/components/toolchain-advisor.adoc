...
* **Repository:** `https://github.com/dev-centr/toolchain-advisor`
* **Catalog file:** `catalog/advisor.json`
* **Shared logic (web):** `lib/src/index.ts` — parse, rank, pick best (`@dev-centr/toolchain-advisor-core`)
...
| **devcentr.org**
| Build: CI checks out `toolchain-advisor`, copies catalog to `public/catalog/advisor.json`. Runtime: fetches `catalog/advisor.json` from raw GitHub `main`, then falls back to bundled `/catalog/advisor.json`.

| **Contributors**
| PR against `toolchain-advisor`; CI validates JSON parses via `lib` parser.
|===
...
* **Web:** `https://devcentr.org/toolchain-advisor` (same catalog and ranking rules).
