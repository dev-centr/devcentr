...
It sits on top of xref:components/project-recognizer.adoc[Project Recognizer] (stack detection via `project-map`) and xref:components/dependency-modeler.adoc[Dependency Modeler] (intra- and inter-project graphs). Recognition alone lists stacks; the visualizer **teaches the mental model** and **routes actions** from the repo root.
...
The visualizer is the **cognitive projection** for *project internals* — analogous to xref:components/role-topology-composer.adoc[Role Topology Composer] for layered AI stacks, but scoped to dependency/control structure and dev-tool command routing inside a codebase.
...
xref:devcentr:project-vision.adoc[Project vision] already requires both:
...
* xref:components/project-recognizer.adoc[Project Recognizer] — stack detection; feeds `stackRef` matching.
* xref:components/dependency-modeler.adoc[Dependency Modeler] — module and external dependency graphs.
* xref:components/role-topology-composer.adoc[Role Topology Composer] — layered *environment* stacks (AI, toolchains); orthogonal to per-repo blueprint.
