= JSON5 Endorsement
:description: Use JSON5 when tools require JSON; prefer SDL for human-edited config when possible.
...
When you control the parser (e.g. Dev Center app config, recognizer profiles), prefer xref:sdl.adoc[SDL (SDLang)] over JSON5. Use JSON5 when a tool or API strictly requires JSON (e.g. `package.json`, VS Code `settings.json`, dprint/biome config).
...
* xref:sdl.adoc[SDL (SDLang) preference] — Prefer SDL for settings and config when the parser supports it.
