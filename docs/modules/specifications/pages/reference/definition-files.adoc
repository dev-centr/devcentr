...
* **Package manager fields:** `id`, `name`, optional `status` (`preferred`, `active`, `bundled`, `closed-aspirational`, …), `notes`, `homepage`. Node’s catalog includes **Orogene** as closed-aspirational inspiration and **Bun** as a practical OSS alternative; narrative: xref:general-knowledge:explanation/infrastructure/npm-package-managers.adoc[npm package managers].
...
* **Spec:** xref:../components/content-create.adoc[Content Create]
...
* **Schema:** `app/src/modules/project-visualizer/interaction-layouts/README.adoc`
* **Purpose:** Human-authored **mental models** for dev tools — UI layout zones, command grammar (where each token comes from), root-first routing flags (`--filter`, `-C`, `cargo -p`, …), and copy/run presets. Consumed by xref:../components/project-visualizer.adoc[Project Visualizer]; overlaid with repo-specific facts from extractors (script keys, workspace members).
* **Initial catalog:** node, npm, pnpm, vite, next, solid, dub, rust (hand-maintained) plus **141** generated stubs from `interaction-layouts/_catalog.tsv` (**149** total). Regenerate: `_generate-from-tsv.ps1`.
* **Not the same as:** `languages/*.sdl` (install/toolchain catalog) or `project-map` stack profiles (detection only).
* **Reference prose example:** xref:general-knowledge:reference/pnpm-run-routing.adoc[pnpm run routing].
