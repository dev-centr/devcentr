...
**KDL** (KDL Document Language) is a modern node-based format in the same family as SDLang. It emphasizes a strict unambiguous grammar, `/-` node commenting, and parenthetical type annotations such as `(u64)123`. Many treat it as a successor narrative to SDLang for greenfield human-readable documents.

DevCentr still **prefers SDLang** for settings and catalogs it owns (`sdlang-d`, existing `.sdl` profiles, Toolchain Advisor / Content Create definitions). Choose KDL when the consuming toolchain has first-class KDL support and the file is not parsed by DevCentr’s SDLang stack.
