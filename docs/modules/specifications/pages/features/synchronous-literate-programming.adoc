...
Runnable **playbooks** (install/setup on a machine) are a different track: xref:features/scriptbook-system-playbooks.adoc[Scriptbook]. Do not confuse `::: tangle` with `::: playbook`. Illustrated model: xref:general-knowledge::explanation/infrastructure/abstract-playbooks.adoc[Abstract playbooks].
