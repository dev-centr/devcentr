...
The manager does not need to implement each framework's loader; it only needs to **output** valid KEY=value lines (and optional comments) so that the framework's own loader can read them. Provider and tool profiles (see xref:reference/provider-tool-profiles.adoc[Provider and Tool Profiles]) can define the **variable names** that each service or framework expects (e.g. `DATABASE_URL`, `STRIPE_SECRET_KEY`), so that when you attach a secret to a project, the manager can suggest or use the right name in `.env`.
