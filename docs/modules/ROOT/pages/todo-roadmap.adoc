...
* [ ] **Project Visualizer (Live Blueprint)**
** [ ] SDL loader for `project-visualizer/interaction-layouts/*.sdl`
** [ ] Merge profiles with `project-map` scan + repo extractors (scripts, workspace members)
** [ ] Canvas: dependency graph + control graph toggles
** [ ] Root-first command presets from project root (no nested cd in primary UX)
** [x] Initial interaction layout profile catalog (8 starters + 141 generated = 149; see `interaction-layouts/_catalog.tsv`)
** [ ] Spec: xref:specifications:components/project-visualizer.adoc[Project Visualizer]
...
** [x] Spec: PlayTime engine vs Scriptbook app vs CentrMark; intent steps (not shell translation); Host Awareness bake loop.
** [x] PlayTime intent bind for `cli.install` + sidecar resolved command (scriptbook library).
...
* [ ] **Fonts / Appearance**
** [x] Five OS-font radios + editable preview + Scriptbook install (Cascadia Mono default, JetBrains Mono, Fira Code, Iosevka, GitHub Monaspace). Ligature demo line is preview-only (dlangui cannot apply OpenType ligatures).
** [ ] Optional: Nerd Font as a distinct CLI-glyphs / patch toggle (not another radio).
** [ ] Project font lock library (Fontsource packages + OFL in-tree); DevCentr is the first client.
** [ ] Clearer UX when Scriptbook is missing; more Windows winget methods in the catalog.
...
* **Fonts / Appearance:** OS-font picker for *this app* is shipped (five coding monos, Scriptbook install). Project Fontsource lock and a Suitcase-style manager are later / separate -- not forgotten. Spec: xref:specifications:components/appearance.adoc[Appearance (code font)].
